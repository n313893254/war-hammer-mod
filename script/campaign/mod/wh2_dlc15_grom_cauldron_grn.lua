local grom_faction_key = ""
local grom_faction_name = "wh2_dlc15_grn_broken_axe"
...
function add_grom_food_listeners_grn()
	out("#### Adding Grom Cauldron Listeners grn ####")
	...
	core:remove_listener("ai_grom_cooks_recipe");
	core:remove_listener("ai_grom_increases_trait");
	if is_new_game then
	...
	for ingredient, data in pairs(ingredients_data) do
		if not cooking_interface:is_ingredient_unlocked(ingredient) then
			if data.start_unlocked then
				unlock_ingredient_grn(ingredient)
			end
			
			if data.ancillaries_required then
				core:add_listener(
					"grn_ingredient_ancillary_" .. ingredient,
			...
			if data.waaagh_unlocks then
				core:add_listener(
					"grn_ingredient_waaagh_" .. ingredient,
			...
			if data.mission_unlock then
				core:add_listener(
					"grn_ingredient_mission_" .. ingredient,
					"MissionSucceeded",
					function(context)
                ...
                if data.mission_unlock ==  "wh3_main_ie_qb_grn_grom_axe_of_grom" then
					grom_cqi_resign();
                    core:add_listener(
                            "food_grn_eight",
                            "FactionTurnStart",
                            ...
                            function(context)
                                local character = cm:get_character_by_cqi(Grom_cqi);
                                if  character:has_ancillary("wh2_dlc15_anc_weapon_axe_of_grom") then
                                    unlock_ingredient_grn(ingredient);
                                end
                            end,
                            false
			...
			if data.waaagh_elf_trophy_unlocks then
				core:add_listener(
					"grn_ingredient_waaagh_elf_trophy_" .. ingredient,
			...
			if data.heroes_to_rank_up then
				core:add_listener(
					"grn_ingredient_rank_up_" .. ingredient,
					"CharacterRankUp",
					function(context)
			...
			if data.subculture_to_sack then
				core:add_listener(
					"grn_ingredient_sack_" .. ingredient,
			...
			if data.units_to_recruit then
				core:add_listener(
					"grn_ingredient_unit_recruited_" .. ingredient,
					"UnitTrained",
					function(context)
			...
			if data.units_to_defeat then
				core:add_listener(
					"grn_ingredient_units_defeated_" .. ingredient,
					"BattleCompleted",
					function()
	...
	core:add_listener(
		...
		function()
			...
			if #available_ingredients_to_drop > 0 then
				...
				unlock_ingredient_grn(available_ingredients_to_drop[cm:random_number(#available_ingredients_to_drop)])
			end
		end,
		true
	)
	...
	if current_food_challenge.is_active then
		setup_food_challenge_listener_grn()
	end
	
	-- food merchant interacted with
	core:add_listener(
		"grn_area_entered_trigger_food_merchant",
		"AreaEntered",
		function(context)
		...
		function(context)
			...
			if not character:is_null_interface() and character:faction():name() == grom_faction_key then
				if get_number_of_ingredients_grn(false) > 0 then
				...
				core:add_listener(
					"grn_foodmerchant_dilemma_choice",
					...
					function(context)
						...
						if dilemma == merchant_dilemma_all_ingredients then
							if choice == 0 then
								create_new_food_challenge_grn()
							elseif choice == 1 then
								cook_random_recipe_grn()
							end
...
function cook_random_recipe_grn()
...
function unlock_ingredient_grn(ingredient)
...
function check_blacktoof_mission_requirement_grn()
	if get_number_of_ingredients_grn(true) >= blacktoof_required_ingredients_number then
...
function spawn_food_merchant_grn(queue_time)
...
function get_number_of_ingredients_grn(unlocked)
	local count = 0
...
function create_new_food_challenge_grn()
	local cooking_interface = cm:model():world():cooking_system():faction_cooking_info(cm:get_faction(grom_faction_key))
...
function setup_food_challenge_listener_grn()
	core:add_listener(
		"grn_food_challenge_listener_" .. tostring(current_food_challenge.id),
		"FactionCookedDish", 
		function(context)
...
local function show_grom_cook_button(state)
 
	local ui_root = core:get_ui_root();	
	out("shy0 show_grom_cook_button print:");
	local ui_sotek = find_uicomponent(ui_root,"hud_campaign", "faction_buttons_docker", "button_group_management", "button_groms_cauldron");
	out("shy0 show_grom_cook_button print:1");
	if not ui_sotek then
	out("shy0 show_grom_cook_button ERROR: button find failed!");	
		return false;	
	end;		
...
local function show_grom_cook_holder(state)
		
	local ui_root = core:get_ui_root();	
	out("shy1 show_grom_cook_holder print:");
	local ui_sotek = find_uicomponent(ui_root,"grom_dish_effect_holder");
	out("shy1 show_grom_cook_holder print:1");
	if not ui_sotek then
	out("shy1 show_grom_cook_holder ERROR: button find failed!");	
		return false;	
	end;		
...
local function grn_resetup_grom_cook_dish()	
	
	out("grom :"..grom_faction_key);
	out("grom :"..grom_faction_name);
	--giving Grom his starting trait
	if grom_faction_key ~= grom_faction_name then	
		local faction_interface = cm:get_faction(grom_faction_key);	
		if not faction_interface or faction_interface:is_dead() then
			out("grom error no interface");
			return;
		end	
		local faction_leader_cqi = faction_interface:faction_leader():command_queue_index();
		out("faction leader cqi is "..faction_leader_cqi);
		cm:force_add_trait("character_cqi:"..faction_leader_cqi, food_trait, true);
		out("faction leader1 cqi is "..faction_leader_cqi);
		local char_interface = cm:get_character_by_cqi(faction_leader_cqi);		
		local current_lvl = char_interface:trait_points(food_trait);	
		out("current_lvl "..current_lvl);
		
	end	
 --   UnlockFood_grn(food_index[5]);
	out("setup trait end");
end
...
function grom_cqi_resign()
	...
	local faction_char = faction_interface:character_list();
	
	for i = 0, faction_char:num_items() - 1 do
		local this_char = faction_char:item_at(i);
			
		if this_char:character_subtype("wh2_dlc15_grn_grom_the_paunch") == true then
			out("grom cqi before is "..Grom_cqi);
			Grom_cqi = this_char:command_queue_index();	
			out("grom cqi after is "..Grom_cqi);
            return;
		end
	end
...
cm:add_first_tick_callback(
        function()
            if grom_faction_key ~= "" then
                core:remove_listener("ai_grom_cooks_recipe");
                core:remove_listener("ai_grom_increases_trait");
            end            
        end);

function  wh2_dlc15_grom_cauldron_grn()
	local_faction = cm:get_local_faction_name(true);		
	out("wh2_dlc15_grom_cauldron_greenskins:local_faction "..local_faction);
	local script_belongs_faction = cm:model():world():faction_by_key(local_faction);   
	
	if script_belongs_faction:culture() ==  "wh_main_grn_greenskins" and grom_faction_key ~= "" and local_faction == grom_faction_key then		
		show_grom_cook_button(true);
		show_grom_cook_holder(true);
--		grom_resign_all();		
		add_grom_food_listeners_grn();
--		set_local_grom_faction(grom_faction_key);
		add_grom_story_listeners_grn(grom_faction_key);
    elseif  script_belongs_faction:culture() ==  "wh_main_grn_greenskins" and grom_faction_key == "" and local_faction ~= grom_faction_name then
        show_grom_cook_button(false);
		show_grom_cook_holder(false);
	end	
	
	core:add_listener(
		"confederation_grom_grn_faction",
		"FactionJoinsConfederation",
		function(context)
			local faction_name = context:faction():name();
			out("grom:faction_name"..faction_name);
			return faction_name == grom_faction_name;
		end,
		function(context)
			local faction = context:confederation();
         --   local confederation_faction = context:confederation();
			local confederation_name = context:confederation():name();
			out("grom:confederation_name"..confederation_name);
			local faction_name_log = context:faction():name();
			out("grom:faction_name_log"..faction_name_log);
			if faction:is_human() and grom_faction_key == "" then
				grom_faction_key = confederation_name;
				show_grom_cook_button(true);
				show_grom_cook_holder(true);
				add_grom_food_listeners_grn();
				grn_resetup_grom_cook_dish();
--				set_local_grom_faction(grom_faction_key);
				add_grom_story_listeners_grn(grom_faction_key);
			--	core:remove_listener("AI_food_eating");			
			else
				out("grom:fation is not hunman");
			end;
		end,
		true
	);
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("grom_faction_key", grom_faction_key, context)
		cm:save_named_value("current_food_challenge_grn", current_food_challenge, context)
		cm:save_named_value("cooked_dish_grn", cooked_dish, context)
		cm:save_named_value("dishcookedhag_grn", dishcookedhag, context)
	end
)
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
			grom_faction_key = cm:load_named_value("grom_faction_key", "", context)
			current_food_challenge = cm:load_named_value("current_food_challenge_grn", current_food_challenge, context)
			cooked_dish = cm:load_named_value("cooked_dish_grn", cooked_dish, context)
			dishcookedhag = cm:load_named_value("dishcookedhag_grn", dishcookedhag, context)
		end
	end
)