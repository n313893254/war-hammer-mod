...
local taurox_faction_key = "";
local taurox_forename = "names_name_1163393566";
local taurox_faction_name = "wh2_dlc17_bst_taurox";
...
function add_taurox_rampage_listeners_bst()
	out("#### Adding Taurox Rampage Listeners bst ####");
	...
	if taurox_interface then
		core:add_listener(
			"TauroxRampage_FactionTurnStart_bst",
			"FactionTurnStart",
			function(context)
			...
			function(context)
				...
				if cm:turn_number() >= 2 and cm:get_saved_value("taurox_rampage_active_bst") then
					TauroxRampage_MomentumUpdate_bst(momentum_matrix["end_turn"])
				end
			end,
			true
		);
		...
		core:add_listener(
			"TauroxRampage_CharacterCompletedBattle_bst",
			...
			function(context) 
				TauroxRampage_BattleCompleted_bst(context) 
			end,
			true
		);

		core:add_listener(
			"TauroxRampage_CharacterParticipatedAsSecondaryGeneralInBattle_bst",
		...
		core:add_listener(
			"TauroxRampage_CharacterWithdrewFromBattle_bst",
			...
			function(context)
				--Taurox retreated
				TauroxRampage_MomentumUpdate_bst(momentum_matrix["battle_retreat"]);
			end,
			true
		);

		core:add_listener(
			"TauroxRampage_PooledResourceEffectChangedEvent_bst",
		...
		if taurox_interface:is_human() == true  then
			cm:add_faction_turn_start_listener_by_name(
				...
				function(context)
					...
					cm:set_saved_value("taurox_battle_fought_this_turn_bst", false)
				end,
				true
			);
			...
			core:add_listener(
				"TauroxRampage_RitualCompletedEvent_bst",
				...
				function(context) 
					TauroxRampage_RitualCompletedEvent_bst(context) 
				end,
				true
			);

			core:add_listener(
				"TauroxRampage_ComponentLClickUp_bst",
				"ComponentLClickUp",
				function(context)					
			...
			core:add_listener(
				...
				function(context)
					...
					if taurox_faction:has_effect_bundle("wh2_dlc17_taurox_bst_rampage_tier_2_option_4") then					
						TauroxRampage_MomentumUpdate_bst(momentum_matrix["ap_replenish_reduced"]);
					else
						TauroxRampage_MomentumUpdate_bst(momentum_matrix["ap_replenish"]);
					end
				end, 
				true
			);
...
function TauroxRampage_BattleCompleted_bst(context)
	...
	if battle_won == true then
		...
		if attacker_result:find("victory") then
			...
			if (taurox_military_force:unit_list():num_items() == 1) and (hidden_traits["solo_army"].maxed_out == false) and (cm:pending_battle_cache_defender_value() >= army_value_for_solo_trait) then
				TauroxRampage_TraitManager_bst("solo_army");
			end
			...
			if (consecutive_battles_won_this_turn >= multiple_battles_threshold) and (hidden_traits["multiple_wins_same_turn"].maxed_out == false) then
				TauroxRampage_TraitManager_bst("multiple_wins_same_turn");
			...
			if not cm:get_saved_value("taurox_battle_fought_this_turn_bst") then
				battle_type = "battle_won_first_of_turn"
				cm:set_saved_value("taurox_battle_fought_this_turn_bst", true)
			end
			TauroxRampage_MomentumUpdate_bst(momentum_matrix[battle_type]);
		end

		--Apply River of Blood effects
		local region = context:character():region()
		local has_skill = context:character():has_skill(RiverOfBlood_bst.skill_key)
		TauroxRampage_UpdateRiverOfBlood_bst(region, has_skill)

	else
		--Taurox lost a battle
		--call the Momentum Update function with the appropraite value change
		TauroxRampage_MomentumUpdate_bst(momentum_matrix["battle_lost"])
...
function TauroxRampage_MomentumUpdate_bst(momentum_entry)
    if not momentum_entry then return false end
    
	if not cm:get_saved_value("taurox_rampage_active_bst") then
		cm:set_saved_value("taurox_rampage_active_bst", true)
	end
	...
	if current_momentum <= 0 then
		...
		TauroxRampage_RampageRestart_bst(false);
	...
	elseif current_momentum >= high_momentum_threshold and (hidden_traits["high_momentum"].maxed_out == false) then
		...
		TauroxRampage_TraitManager_bst("high_momentum");
...
function TauroxRampage_PooledResourceEffectChangedEvent_bst()
	...
	if current_active_effect ~= "no_bundle" then 
		...
		TauroxRampage_RitualManager_bst(current_rampage_tier, taurox_faction, true);
	...
	if (current_rampage_tier == 3)  then
		TauroxRampage_RampageRestart_bst(true);

		if (hidden_traits["rampage_complete"].maxed_out == false) then
			-- add a tier onto the HIDDEN TRAIT
			TauroxRampage_TraitManager_bst("rampage_complete")
		end
	end
end

--Controls Locking/Unlocking of rituals based on bst_rampage threshold
function TauroxRampage_RitualManager_bst(current_tier, faction_interface, should_unlock)
...
function TauroxRampage_RitualCompletedEvent_bst(context)
	...
	--call TauroxRampage_RitualManager_bst to lock all the other rituals AND re-enable end turn functionality
	TauroxRampage_RitualManager_bst(rampage_tier, taurox_faction, false)
...
function TauroxRampage_TraitManager_bst(trait)
...
function TauroxRampage_RampageRestart_bst(victory)
	local taurox_faction = cm:get_faction(taurox_faction_key);
	...
	cm:set_saved_value("taurox_rampage_active_bst", false)
...
function TauroxRampage_UpdateRiverOfBlood_bst(region, has_skill)
	...
	for i, bundle_key in ipairs(RiverOfBlood_bst.bundle_keys) do
	...
	if has_skill then
		bundle_level = bundle_level + RiverOfBlood_bst.skill_bonus_stacks
	end

	if bundle_level > #RiverOfBlood_bst.bundle_keys then
		bundle_level = #RiverOfBlood_bst.bundle_keys
	end

	cm:apply_effect_bundle_to_region(RiverOfBlood_bst.bundle_keys[bundle_level], region_key, RiverOfBlood_bst.duration)

end
local function show_bst_taurox_rampage_holder(state)
		
	local ui_root = core:get_ui_root();	
	out("shy1 show_bst_taurox_rampage_holder print:");
	local ui_sotek = find_uicomponent(ui_root,"hud_campaign","taurox_rampage_holder");
	out("shy1 show_bst_taurox_rampage_holder print:1");
	if not ui_sotek then
	out("shy1 show_bst_taurox_rampage_holder ERROR: button find failed!");	
		return false;	
	end;		
...
local function remove_taurox_listeners()
    core:remove_listener("TauroxRampage_FactionTurnStart");
    core:remove_listener("TauroxRampage_CharacterCompletedBattle");
    core:remove_listener("TauroxRampage_CharacterParticipatedAsSecondaryGeneralInBattle");
    core:remove_listener("TauroxRampage_CharacterWithdrewFromBattle");
    core:remove_listener("TauroxRampage_PooledResourceEffectChangedEvent");
    core:remove_listener("TauroxRampage_RitualCompletedEvent");
	core:remove_listener("TauroxRampage_ComponentLClickUp");
	core:remove_listener("TauroxRampage_FactionTurnStartAI");
end

cm:add_first_tick_callback(
        function()
            if taurox_faction_key ~= "" then
                remove_taurox_listeners();
            end             
        end);

function wh2_dlc17_taurox_rampage_bst()
	local_faction = cm:get_local_faction_name(true);		
	out("wh2_dlc17_taurox_rampage_bst:local_faction "..local_faction);
	local script_belongs_faction = cm:model():world():faction_by_key(local_faction);   
	
    
	if script_belongs_faction:culture() ==  "wh_dlc03_bst_beastmen" and taurox_faction_key ~= "" and local_faction == taurox_faction_key then		
		show_bst_taurox_rampage_holder(true);
        remove_taurox_listeners();
		add_taurox_rampage_listeners_bst();
    elseif  script_belongs_faction:culture() ==  "wh_dlc03_bst_beastmen" and taurox_faction_key == "" and local_faction ~= taurox_faction_name then
        show_bst_taurox_rampage_holder(false);  
--		cm:faction_add_pooled_resource(local_faction, "bst_dread", "wh2_dlc17_bst_dread_gain_taurox_rampage", 9999);
	end	
	
	core:add_listener(
		"confederation_bst_taurox_faction",
		"FactionJoinsConfederation",
		function(context)
			local faction_name = context:faction():name();
			out("bst:faction_name"..faction_name);
			return faction_name == taurox_faction_name;
		end,
		function(context)
			local faction = context:confederation();
         --   local confederation_faction = context:confederation();
			local confederation_name = context:confederation():name();
			out("bst:confederation_name"..confederation_name);
			local faction_name_log = context:faction():name();
			out("bst:faction_name_log"..faction_name_log);
			if faction:is_human() and taurox_faction_key == "" then
				taurox_faction_key = confederation_name;
				show_bst_taurox_rampage_holder(true);
                remove_taurox_listeners();
				add_taurox_rampage_listeners_bst();				
--				cm:faction_add_pooled_resource(taurox_faction_key, "bst_rampage", "wh2_dlc17_taurox_bst_rampage_gain_battles", 49);		
			else
				out("bst:fation is not hunman");
			end;
		end,
		true
	);
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("taurox_faction_key", taurox_faction_key, context);
		cm:save_named_value("momentum_matrix_bst", momentum_matrix, context);
		cm:save_named_value("hidden_traits_bst", hidden_traits, context);
		cm:save_named_value("consecutive_battles_won_this_turn_bst", consecutive_battles_won_this_turn, context);	
		cm:save_named_value("base_battle_rampage_bst", base_battle_rampage, context);
		cm:save_named_value("completed_rampage_rituals_bst", completed_rampage_rituals, context);
	end
);
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
			taurox_faction_key = cm:load_named_value("taurox_faction_key", "", context);
			momentum_matrix = cm:load_named_value("momentum_matrix_bst", momentum_matrix, context);
			hidden_traits = cm:load_named_value("hidden_traits_bst", hidden_traits, context);
			consecutive_battles_won_this_turn = cm:load_named_value("consecutive_battles_won_this_turn_bst", consecutive_battles_won_this_turn, context);		
			base_battle_rampage = cm:load_named_value("base_battle_rampage_bst", base_battle_rampage, context);
			completed_rampage_rituals = cm:load_named_value("completed_rampage_rituals_bst", completed_rampage_rituals, context);
		end
	end
);
