local m_sisters_faction_key = ""
local m_favour_on_cooldown = false
local m_sisters_faction_name_key = "wh2_dlc16_wef_sisters_of_twilight"
local m_forge_items = {
    -- item_table_keys
    "dragon_cuirass",
    "eagle_vambraces",
...
function add_sisters_forge_listeners_wef()
	...
	core:add_listener(
		"Sisters_ReinforcementFavourGain_wef",
	...
	core:add_listener(
		"Sisters_ForgeRitualCompleted_wef",
...
function show_wef_daith_holder(state)
		
	local ui_root = core:get_ui_root();	
	out("shy1 show_wef_daith_holder print:");
	local ui_sotek = find_uicomponent(ui_root,"forge_of_daith_holder");
	out("shy1 show_wef_daith_holder print:1");
	if not ui_sotek then
        out("shy1 show_wef_daith_holder ERROR: button find failed!");	
		return;	
	end;		
...
function add_sisters_volley_of_kurnous()
    local effect_key = "wh2_dlc16_lord_trait_wef_sisters_of_twilight";
    if m_sisters_faction_key ~= "" then
        local enable_faction = cm:model():world():faction_by_key(m_sisters_faction_key);
		if not enable_faction:has_effect_bundle(effect_key) then			
			cm:apply_effect_bundle(effect_key, m_sisters_faction_key, 0);							
		end;
    end
end


function  wh2_dlc16_wef_sisters_forge_wef()
	local_faction = cm:get_local_faction_name(true);		
	out("wh2_dlc16_wef_sisters_forge_wef:local_faction "..local_faction);
	local script_belongs_faction = cm:model():world():faction_by_key(local_faction);   
	
	if script_belongs_faction:culture() ==  "wh_dlc05_wef_wood_elves" and m_sisters_faction_key ~= "" and local_faction == m_sisters_faction_key then		
		show_wef_daith_holder(true);
		add_sisters_forge_listeners_wef();
        add_sisters_volley_of_kurnous();
    elseif  script_belongs_faction:culture() ==  "wh_dlc05_wef_wood_elves" and m_sisters_faction_key == "" and local_faction ~= m_sisters_faction_name_key then
        show_wef_daith_holder(false);        
	end	
	
	core:add_listener(
		"confederation_wef_sister_faction",
		"FactionJoinsConfederation",
		function(context)
			local faction_name = context:faction():name();
			out("wef:faction_name"..faction_name);
			return faction_name == m_sisters_faction_name_key;
		end,
		function(context)
			local faction = context:confederation();
         --   local confederation_faction = context:confederation();
			local confederation_name = context:confederation():name();
			out("wef:confederation_name"..confederation_name);
			local faction_name_log = context:faction():name();
			out("wef:faction_name_log"..faction_name_log);
			if faction:is_human() and m_sisters_faction_key == "" then
				m_sisters_faction_key = confederation_name;
				show_wef_daith_holder(true);
				add_sisters_forge_listeners_wef();		
                add_sisters_volley_of_kurnous();
---				core:remove_listener("Sisters_AiBehaviour");			
			else
				out("wef:fation is not hunman");
			end;
		end,
		true
	);
...
cm:add_saving_game_callback(
    function(context)
		cm:save_named_value("m_sisters_faction_key", m_sisters_faction_key, context)
		cm:save_named_value("sisters_forge_items_wef", m_forge_items, context)
        cm:save_named_value("sisters_current_forge_cooldown_wef", m_favour_on_cooldown, context)
		cm:save_named_value("sisters_script_context_wef", m_script_context_chosen, context)
	end
)
...
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
			m_sisters_faction_key = cm:load_named_value("m_sisters_faction_key", "", context)
            m_forge_items = cm:load_named_value("sisters_forge_items_wef", m_forge_items, context)
            m_favour_on_cooldown = cm:load_named_value("sisters_current_forge_cooldown_wef", m_favour_on_cooldown, context)
			m_script_context_chosen = cm:load_named_value("sisters_script_context_wef", m_script_context_chosen, context)
		end
	end
)