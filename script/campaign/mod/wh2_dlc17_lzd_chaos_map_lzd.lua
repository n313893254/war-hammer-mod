...
local m_vamp_count_culture = "wh2_main_hef_high_elves"
...
local function update_blocked_region_list()
	...
	if(Silent_Sanctums_lzd:get_sanctum_transport_region() ~= "") then
		local region = cm:get_region(Silent_Sanctums_lzd:get_sanctum_transport_region())
		local adjacent_region_list = region:adjacent_region_list()
...
local function grant_sanctum_gems(mission_key)
	...
	Silent_Sanctums_lzd:add_sanctum_gems(amount)
end
...
local function oxyotl_start_threat_map_feature_listener()
	core:add_listener(
		...
		function(context)
			local faction = context:faction():name()
			return faction == m_oxyotl_faction_key and m_start_mission == true
		end,
		function(context)
			out("start starting mission!")
...
local function oxyotl_remove_tresspass_listener()
	core:add_listener(
		"Oxy_RemoveTresspassPenalty_lzd",
		"FactionTurnStart",
		function(context)
			local faction = context:faction():name()
			return faction == m_oxyotl_faction_key and m_start_mission == true
		...
		function(context)
			...
			for i = 0, faction_char:num_items() - 1 do
				...
				if this_char:character_subtype("wh2_dlc17_lzd_oxyotl") == true then
				   -- out("oxy cqi before is "..Grom_cqi);
					local oxyotlo = this_char;	
					cm:set_character_excluded_from_trespassing(oxyotlo, true)
					
			   --     out("oxy cqi after is "..oxyotl);			
				end
			end	
		end,
		true
...
local function oxyotl_setup_mission_markers_listener()
	core:add_listener(
		"Oxy_RemoveTresspassPenalty_lzd1",
		"FactionTurnStart",
		function(context)
			local faction = context:faction():name()
			return faction == m_oxyotl_faction_key and m_start_mission == true
		...
		function(context)
			out("start oxyotl_setup_mission_markers_listener!")
...
function add_oxyotl_threat_map_listeners_lzd(faction_key)
	out("wh2_dlc17_lzd_chaos_map_lzd begin!");
	m_oxyotl_faction_key = faction_key
	local oxyotl_faction = cm:get_faction(m_oxyotl_faction_key)
...
function chaos_map_remove_listener()
	core:remove_listener("Oxy_RetreatCheck")
	core:remove_listener("Oxy_PostInvasionBattle")
	core:remove_listener("Oxy_ThreatMapMissionVision")
	core:remove_listener("oxy_force_raid_stance")
	core:remove_listener("threat_map_manager")
	core:remove_listener("Oxy_ThreatMapArmyAttackDilemmaChoice")
	core:remove_listener("Oxy_ThreatMapMakerInteraction")
	core:remove_listener("Oxy_ThreatMapMissionSuccess")
	core:remove_listener("Oxy_ThreatMapMissionCancelled")
	core:remove_listener("Oxy_ThreatMapMissionFailed")
	core:remove_listener("Oxy_ThreatMapActivate")
	core:remove_listener("Oxy_RemoveTresspassPenalty")
	core:remove_listener("Oxy_HomeRegionUpdate")
	core:remove_listener("Oxy_RemoveTresspassPenalty")
end
cm:add_first_tick_callback(
        function()
            if m_oxyotl_faction_key ~= "" then
                chaos_map_remove_listener();
            end              
        end);
function wh2_dlc17_lzd_chaos_map_lzd()
	out("wh2_dlc17_lzd_chaos_map_lzd start");
end
...
cm:add_saving_game_callback(
    function(context)
		cm:save_named_value("m_start_mission", m_start_mission, context)
		cm:save_named_value("oxy_map_missions_started_lzd", m_threat_map_missions_started, context)
		cm:save_named_value("oxy_map_last_interaction_cqi_lzd", m_latest_marker_interaction_cqi, context)
		cm:save_named_value("oxy_map_invasion_count_lzd", m_invasion_count, context)
		cm:save_named_value("oxy_map_active_missions_lzd", m_active_mission_details, context)
		cm:save_named_value("oxy_map_active_consequences_lzd", m_active_mission_consequences, context)
		cm:save_named_value("oxy_map_active_rewards_lzd", m_active_mission_rewards, context)
		cm:save_named_value("oxy_map_blocked_regions_lzd", m_blocked_region_list, context)
		cm:save_named_value("oxy_map_region_marker_details_lzd", m_region_marker_mission_details, context)
		cm:save_named_value("oxy_map_sanctum_marker_details_lzd", m_sanctum_marker_mission_details, context)
		cm:save_named_value("oxy_map_wave_details_lzd", m_mission_wave_details, context)
		cm:save_named_value("oxy_scripted_battle_details_lzd", m_scripted_battle_active, context)
		cm:save_named_value("oxy_diplomatic_mission_reward_amounts_lzd", m_diplomatic_mission_reward_amounts, context)
	end
)
...
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
			m_start_mission = cm:load_named_value("m_start_mission", m_start_mission, context)
			m_threat_map_missions_started = cm:load_named_value("oxy_map_missions_started_lzd", m_threat_map_missions_started, context)
			m_latest_marker_interaction_cqi = cm:load_named_value("oxy_map_last_interaction_cqi_lzd", m_latest_marker_interaction_cqi, context)
			m_invasion_count = cm:load_named_value("oxy_map_invasion_count_lzd", m_invasion_count, context)
			m_active_mission_details = cm:load_named_value("oxy_map_active_missions_lzd", m_active_mission_details, context)
			m_active_mission_consequences = cm:load_named_value("oxy_map_active_consequences_lzd", m_active_mission_consequences, context)
			m_active_mission_rewards = cm:load_named_value("oxy_map_active_rewards_lzd", m_active_mission_rewards, context)
			m_blocked_region_list = cm:load_named_value("oxy_map_blocked_regions_lzd", m_blocked_region_list, context)
			m_region_marker_mission_details = cm:load_named_value("oxy_map_region_marker_details_lzd", m_region_marker_mission_details, context)
			m_sanctum_marker_mission_details = cm:load_named_value("oxy_map_sanctum_marker_details_lzd", m_sanctum_marker_mission_details, context)
			m_mission_wave_details = cm:load_named_value("oxy_map_wave_details_lzd", m_mission_wave_details, context)
			m_scripted_battle_active = cm:load_named_value("oxy_scripted_battle_details_lzd", m_scripted_battle_active, context)
			m_diplomatic_mission_reward_amounts = cm:load_named_value("oxy_diplomatic_mission_reward_amounts_lzd", m_diplomatic_mission_reward_amounts, context)
		end
	end
)