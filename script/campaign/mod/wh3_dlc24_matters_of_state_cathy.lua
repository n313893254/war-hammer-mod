local matters_of_state_cathy_culture_key = "wh3_main_cth_cathay"
local matters_of_state_cathy_faction = "wh3_dlc24_cth_the_celestial_court"
local matter_of_state_first_init = false;

matters_of_state_cth = {
	faction_string = "",
...
function matters_of_state_cth:initialise()
	if not cm:get_faction(self.faction_string) then return false end

	out("#### Adding Matters of State cth Listeners ####")

	if matter_of_state_first_init == false and cm:is_faction_human(self.faction_string) then
		...
		matter_of_state_first_init = true;
	end
	
	core:add_listener(
		"matters_of_state_cth_FactionTurnStart",
		"FactionTurnStart",
		function(context)
	...
	core:add_listener(
		"matters_of_state_cth_BattleCompleted",
		"BattleCompleted",
		function()
    ...
    core:add_listener(
		"matters_of_state_cth_pr_expended",
    ...
    core:add_listener(
		"matters_of_state_cth_RitualCompletedEvent",
	...
	local function mos_hero_action_success_cth(context)
	...
	core:add_listener(
		"JadeDragonHeroActionListener_cth",
		"CharacterCharacterTargetAction",
		function(context)
			return context:target_character():faction():name() ~= self.faction_string
		end,
		function(context)
			mos_hero_action_success_cth(context)
		end,
		true
	)

	core:add_listener(
		"JadeDragonHeroActionListenerSettlement_cth",
	...
	if cm:get_faction(self.faction_string):is_human() then
		if not cm:get_saved_value("yuan_bo_victory_complete_cth") then
			core:add_listener(
				"Yuan_Bo_Completes_Relay_cth",
				"BuildingCompleted",
				function(context)
				...
				function(context)
					...
					if not cm:get_saved_value("cth_yuan_tower_" .. direction) then
			...
			for direction, direction_table in pairs(self.compass_directions) do
				core:add_listener(
					"cthMattersOfStateIncursionTimer" .. direction,
					"FactionTurnStart",
					function(context)
						return context:faction():name() == self.faction_string and (cm:get_saved_value("cth_yuan_bo_incursion_countdown_" .. direction) or 0) > 0
					end,
					function()
						local countdown = (cm:get_saved_value("cth_yuan_bo_incursion_countdown_" .. direction) or self.incursion_variables.incursion_duration) - 1
						cm:set_saved_value("cth_yuan_bo_incursion_countdown_" .. direction, countdown)
						self:update_incursion_context_values()
						if countdown == 0 then
				...
				core:add_listener(
					"cthMattersOfStateIncursionFailure" .. direction,
					...
					function(context)
						...
						return (cm:get_saved_value("cth_yuan_bo_incursion_countdown_" .. direction) or 0) > 0 and context:previous_owner() == self.faction_string and context:garrison_residence():region():name() == self.compass_directions[direction][cm:get_campaign_name()].region
					end,
					function(context)
						cm:set_saved_value("cth_yuan_bo_incursion_countdown_" .. direction, 0)
						self:update_incursion_context_values()
		...
		core:add_listener(
			"ScriptEventYuanBoIncursionMarkerExpired_cth",
			...
			function(context)
				...
				matters_of_state_cth:trigger_invasion(x, y, direction)
			end,
			true
		)

		core:add_listener(
			"ScriptEventYuanBoIncursionMarkerInteraction_cth",
			...
			function(context)
				...
				local escalation = matters_of_state_cth.incursion_variables.current_escalation
				local invasion_power = matters_of_state_cth.incursion_variables.escalation_level[escalation].army_power
				local invasion_size = matters_of_state_cth.incursion_variables.escalation_level[escalation].army_size
				...
				Forced_Battle_Manager:trigger_forced_battle_with_generated_army(
					...
					invasion_size + matters_of_state_cth.incursion_variables.army_size_interruption_modifiers[string.gsub(marker_ref, direction_key .. "_", "")],
					invasion_power,
					false,
					false,
					true,
		...
		core:add_listener(
			"yuan_bo_final_battle_victory_cth",
			"MissionSucceeded",
			function(context)
...
function matters_of_state_cth:increment_timer()
...
function matters_of_state_cth:update_capacity(pr)
...
function matters_of_state_cth:ritual_completed_event(ritual)
...
function matters_of_state_cth:trigger_invasion(x, y, direction)
...
function matters_of_state_cth:start_incursion(direction) --starts an incursion event for a specific compass direciton
	local countdown = cm:get_saved_value("cth_yuan_bo_incursion_countdown_" .. direction) or 0
	
	if countdown < 1 then
		cm:set_saved_value("cth_yuan_bo_incursion_countdown_" .. direction, self.incursion_variables.incursion_duration)
		self:update_incursion_context_values()
...
function matters_of_state_cth:incursion_complete(direction)
	...
	cm:set_saved_value("cth_yuan_tower_" .. direction, true)
	
	local relays_built = cm:get_saved_value("yuan_bo_victory_count_cth") or 0
	relays_built = relays_built + 1
	cm:set_saved_value("yuan_bo_victory_count_cth", relays_built)
	...
	if relays_built >= 4 then
		cm:set_saved_value("yuan_bo_victory_complete_cth", true)
		core:remove_listener("Yuan_Bo_Completes_Relay_cth")
...
function matters_of_state_cth:update_incursion_context_values()
	for direction, direction_table in pairs(self.compass_directions) do
		common.set_context_value("compass_incursion_active_" .. direction, cm:get_saved_value("cth_yuan_bo_incursion_countdown_" .. direction) or 0)
	end
end

local function show_matters_of_state(state)
    ui_show_group_management_button("button_matters_of_state",state);
	ui_show_resources_bar_holder_name("jade_compass_directions",state);
	ui_show_resources_bar_holder_name("stone_and_steel_holder",state);
end

function cth_the_celestial_court_faction_trait_set_faction(faction,local_faction)	
	if matters_of_state_cth.faction_string ~= "" then
       return;
    end;
	matters_of_state_cth.faction_string = faction;	
	show_matters_of_state(true);				
	matters_of_state_cth:initialise();
end



function  matters_of_state_cathy_remove_listeners()
    core:remove_listener("matters_of_state_FactionTurnStart");
    core:remove_listener("matters_of_state_BattleCompleted");
    core:remove_listener("matters_of_state_pr_expended");
    core:remove_listener("matters_of_state_RitualCompletedEvent");
    ...
    for direction, direction_table in pairs(matters_of_state_cth.compass_directions) do
        core:remove_listener("MattersOfStateIncursionTimer" .. direction);
		core:remove_listener("MattersOfStateIncursionFailure" .. direction);				
    end
    core:remove_listener("ScriptEventYuanBoIncursionMarkerExpired");
    core:remove_listener("ScriptEventYuanBoIncursionMarkerInteraction");
    core:remove_listener("yuan_bo_final_battle_victory");
end
...
cm:add_first_tick_callback(
        function()
            if matters_of_state_cth.faction_string ~= "" then
                matters_of_state_cathy_remove_listeners();
            end 
...
function  wh3_dlc24_matters_of_state_cathy()
	local_faction = cm:get_local_faction_name(true);		
	out("wh3_dlc24_matters_of_state_cathy:local_faction "..local_faction);
	local script_belongs_faction = cm:model():world():faction_by_key(local_faction);   
	local cathy_culture = matters_of_state_cathy_culture_key;
	if script_belongs_faction:culture() == cathy_culture and matters_of_state_cth.faction_string ~= "" and local_faction == matters_of_state_cth.faction_string then		
		show_matters_of_state(true);
		matters_of_state_cth:initialise();
    elseif  script_belongs_faction:culture() ==  cathy_culture and matters_of_state_cth.faction_string == "" and local_faction ~= matters_of_state_cathy_faction then
        show_matters_of_state(false);
	end	
end
...
cm:add_saving_game_callback(
	function(context)
        cm:save_named_value("matters_of_state_cth_recovery_timer", matters_of_state_cth.recovery_timer, context)
		cm:save_named_value("matters_of_state_cth_pr_stone_pending", matters_of_state_cth.stone_pending, context)
		cm:save_named_value("matters_of_state_cth_pr_steel_pending", matters_of_state_cth.steel_pending, context)
		cm:save_named_value("matters_of_state_cth_pr_maximum_tokens", matters_of_state_cth.maximum_tokens, context)
		cm:save_named_value("matters_of_state_cth_remaining_capacity_upgrades", matters_of_state_cth.remaining_capacity_upgrades, context)
		cm:save_named_value("matters_of_state_cth_incursion_variables", matters_of_state_cth.incursion_variables, context)		
		cm:save_named_value("matters_of_state_cathy_faction_key", matters_of_state_cth.faction_string, context)
		cm:save_named_value("matter_of_state_first_init", matter_of_state_first_init, context)
	end
)
...
cm:add_loading_game_callback(
	function(context)
		if cm:is_new_game() == false then
            matters_of_state_cth.recovery_timer = cm:load_named_value("matters_of_state_cth_recovery_timer", matters_of_state_cth.recovery_timer, context)
			matters_of_state_cth.stone_pending = cm:load_named_value("matters_of_state_cth_pr_stone_pending", matters_of_state_cth.stone_pending, context)
			matters_of_state_cth.steel_pending = cm:load_named_value("matters_of_state_cth_pr_steel_pending", matters_of_state_cth.steel_pending, context)
			matters_of_state_cth.maximum_tokens = cm:load_named_value("matters_of_state_cth_pr_maximum_tokens", matters_of_state_cth.maximum_tokens, context)
			matters_of_state_cth.remaining_capacity_upgrades = cm:load_named_value("matters_of_state_cth_remaining_capacity_upgrades", matters_of_state_cth.remaining_capacity_upgrades, context)
			matters_of_state_cth.incursion_variables = cm:load_named_value("matters_of_state_cth_incursion_variables", matters_of_state_cth.incursion_variables, context)			
			matters_of_state_cth.faction_string = cm:load_named_value("matters_of_state_cathy_faction_key", matters_of_state_cth.faction_string, context)
			matter_of_state_first_init = cm:load_named_value("matter_of_state_first_init", false, context)
		end
	end
)