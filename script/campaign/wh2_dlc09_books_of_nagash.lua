...
function setup_book_missions(faction_key, spawn_forces, is_mp)
	...
	local script_belongs_faction = cm:model():world():faction_by_key(faction_key);
	if books_faction_specific and not is_mp then
		-- Switch the books to specific locations for certain factions in singleplayer
		local fac_culture = script_belongs_faction:culture();
		if book_objective_list_faction[faction_key] ~= nil then
			book_objective_list = book_objective_list_faction[faction_key];
		elseif fac_culture == "wh_main_emp_empire" then
			book_objective_list = book_objective_list_faction["wh3_main_emp_cult_of_sigmar"];
		elseif fac_culture == "wh2_main_hef_high_elves" then
			book_objective_list = book_objective_list_faction["wh3_main_emp_cult_of_sigmar"];
		end
	end
	...
	for i = 1, book_objective_count do
		...
		if(faction_key == "wh2_main_hef_high_elves" or script_belongs_faction:culture() ==  "wh2_main_hef_high_elves") then
			mm:add_payload("effect_bundle{bundle_key wh3_main_books_of_nagash_mannfred_reward_" .. i .. ";turns 0;}");
		elseif(faction_key == "wh3_main_emp_cult_of_sigmar" or script_belongs_faction:culture() ==  "wh_main_emp_empire") then
