...
local mannfred_faction_key = "wh2_main_hef_high_elves"
...
function add_nagash_books_effects_listeners()
	...
	out("wh2_dlc09_books_of_nagash_effects_combi start");
	local local_faction = cm:get_local_faction_name(true);		
	out("wh2_dlc09_books_of_nagash_effects_combi:local_faction "..local_faction);
	local script_belongs_faction = cm:model():world():faction_by_key(local_faction);
    
	
	if script_belongs_faction:culture() ==  "wh_main_emp_empire"  then		
		volkmar_faction_key = local_faction	
	end
	
	if script_belongs_faction:culture() ==  "wh2_main_hef_high_elves"  then		
		mannfred_faction_key = local_faction
	end
